    ...
    if mode == "🔍 Analyze Leaf":
        ...
        with col1:
            ...
            if uploaded_file:
                ...
                with open('leaf_image.jpeg', 'wb') as f:
                    f.write(uploaded_file.getbuffer())
                
                # Display uploaded image
                st.image('leaf_image.jpeg', caption="Uploaded Image")
                
                # Validate image
                is_valid, message = ImagePreprocessor.validate_image('leaf_image.jpeg')
                if not is_valid:
                    st.error(f"❌ {message}")
                else:
            ...
            if uploaded_file:
                ...
                if st.button("🚀 Analyze Leaf", use_container_width=True):
                    with st.spinner("🔄 Analyzing image..."):
                        analysis = analyze_leaf_image('leaf_image.jpeg')
